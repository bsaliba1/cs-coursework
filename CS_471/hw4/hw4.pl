...
/* Problem 0A Answer: */
isBinaryTree(leaf(_)).
isBinaryTree(tree(Left, Right)):- isBinaryTree(Left), isBinaryTree(Right).
...
/* Problem 0C Answer: */
computeS(Op, A1, A2, R):- T=..[Op, A1, A2], R is T.
...
sumlist([], 0).
sumlist([H|T], S):- Ps is S-H, sumlist(T, Ps).
...
sumlist2([], Sum, Sum).
sumlist2([H|T], X, Sum):- Acc is X+H, sumlist2(T, Acc, Sum).
...
/* Problem 5 Answer */
sumPartialR(1,[1]).
sumPartialR(N1, [H|T]) :- H is integer((N1+1)*(N1/2)), N2 is N1-1, sumPartialR(N2, T).
...
sumPartialL(N,Lst):- sumPartialL(N, N, Lst).
sumPartialL(1, M, [M]).
sumPartialL(N, M, [H|T]):- H is M, N1 is N-1, Acc is M+N1, sumPartialL(N1, Acc, T).
...
swap(tree(Left, Right), tree(X, Y)):- swap(Right, X), swap(Left, Y).
...
/* Problem 9 Answer: */
isBinarySearchTree(node(V1, node(V2, L2, R2), node(V3, L3, R3))):- (V1<V3), (V1>V2), isBinarySearchTree(node(V2, L2, R2)), isBinarySearchTree(node(V3, L3, R3)).
isBinarySearchTree(node(V1, leaf, node(V3, L3, R3))):- (V1<V3), isBinarySearchTree(node(V3, L3, R3)).
isBinarySearchTree(node(V1, node(V2, L2, R2), leaf)):- (V1>V2), isBinarySearchTree(node(V2, L2, R2)).
isBinarySearchTree(node(_V1, leaf, leaf)).
...
/* Problem 10 Answer: */
d(X, X, R):- R = 1.
d(C, _X, R):- integer(C), R = 0.
d(C*X, X, R):- R = C.
d(-U, X, R):- d(U,X,R2), R = -R2.
d(U+V, X, R):- d(U, X, R2), d(V, X, R3), R = R2+R3. 
d(U-V, X, R):- d(U, X, R2), d(V, X, R3), R = R2-R3.
d(U*V, X, R):- d(U, X, R2), d(V, X, R3), R = U*R3+V*R2.
d(U^N, X, R):- U = X, C is N-1, R = N*U^C*1.
