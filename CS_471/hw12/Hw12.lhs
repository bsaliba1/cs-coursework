...
Problem 2 Answer:
Need fold because there is no base case that will work for all binary operators

> binOpALL :: Num a => (a -> a -> a) -> [a] -> a
> binOpALL op (x:xs) = foldl (\acc e -> op acc e) x xs
...
Problem 3 Answer:
If in type 'a' was defined as Int then we could use
..>insertAt el n lst = let(h,t) = splitAt n lst in h++[el]++t

> insertAt :: (Eq a, Num a) => b -> a -> [b] -> [b]
> insertAt el n [] = [el]
> insertAt el 0 lst = [el]++lst
> insertAt el n (x:xs) = [x]++insertAt el (n-1) xs
...
> composeList [] v = v
> composeList (x:xs) v = x (composeList xs v)
...
> newtonAppr x guess precision = if abs(x - guess^2) < precision then guess else newtonAppr x ((guess+x/guess)/2) precision
...
> thue [] = []
> thue (x:xs) = [x]++(if x == 0 then [1] else [0]) ++ (thue xs)
...
> replicate' [] = []
> replicate' (x:xs) = (replicate_num x x)++(replicate' xs)
> replicate_num x 0 = []
> replicate_num x n = [x]++(replicate_num x (n-1))
