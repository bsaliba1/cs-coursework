	...
	while(fgets(trace,sizeof(trace),stdin)) {
		...
		if (2==sscanf(trace,"%c %p ",&tt,&taddr)) {
			...
			switch(cache_type) {
				case 'F':
			/*--------------------------------------------------------------------------------------
			Need to figure out if there is already a line set up for this tag						      If not, or if the line is not valid, then you will need to...
				- find a victim - use the LRU algorithm in line_firstUsed()
				- Update the CAM to write the tag to the victim line
				- Invoke the line_setValid() function for the victim								when done, the "line" variable should point to the line that
			matches the current tag.
			-----------------------------------------------------------------------------------------*/
					line=cam_read(f_cam,tag);
					if(!line_isValid(lines,line) || line==-1 ){
						//find victim
						line=line_firstUsed(lines,0,cache_size/cache_blocksize);	
						//update cam with victim line
						cam_write(f_cam,line,tag);
						//set valid for victim line
